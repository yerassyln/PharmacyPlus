	...
	*/
	public function fetchBrandData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if(in_array('viewBrand', $this->permission)) {
				$buttons .= '<button type="button" class="btn btn-warning btn-sm" onclick="editBrand('.$value['id'].')" data-toggle="modal" data-target="#editBrandModal"><i class="fa fa-pencil"></i></button>';	
			}
	...
	public function create()
	{
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	if($create == true) {
        		...
        		$response['messages'] = 'Успешно создан';
        	}
        	else {
	...
	*/
	public function remove()
	{
		...
		if($brand_id) {
			...
			if($delete == true) {
				$response['success'] = true;
				$response['messages'] = "Успешно удален";	
			}
			else {
				$response['success'] = false;
				$response['messages'] = "Ошибка to базе данных при удалении информации о бренде";
			}
		}
		else {
			$response['success'] = false;
