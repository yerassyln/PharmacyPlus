...
defined('BASEPATH') OR exit('Произошла ошибка!!');

class Controller_Element extends Admin_Controller 
{
	...
	*/
	public function index()
	{
		...
		$this->render_template('elements/index', $this->data);	
	...
	*/
	public function fetchAttributeData()
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '<a href="'.base_url('Controller_Element/addvalue/'.$value['id']).'" class="btn btn-default btn-sm"><i class="fa fa-plus"></i> Add value</a> 
	...
	*/
	public function create()
	{
		...
		$this->form_validation->set_rules('attribute_name', 'Element name', 'trim|required');
	...
	*/
	public function update($id)
	{
		...
		if($id) {
			$this->form_validation->set_rules('edit_attribute_name', 'Element name', 'trim|required');
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	if($update == true) {
	        		...
	        		$response['success'] = false;
	        		$response['messages'] = 'Ошибка to базе данных при обновлении информации о бренде';			
	        	}
	        }
	        else {
	        	$response['success'] = false;
			...
			$response['success'] = false;
    		$response['messages'] = 'Ошибка, пожалуйста, обновите страницу снова!';
		}
	...
	*/
	public function remove()
	{
		...
		if($attribute_id) {
			...
			$response['success'] = false;
			$response['messages'] = "Refresh страницу снова !!";
		}
	...
	*/
	public function addvalue($attribute_id = null)
	{
		...
		$this->render_template('elements/addvalue', $this->data);	
	}
